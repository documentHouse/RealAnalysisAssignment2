...
\indent Let $M = \{ (n,m) \; | \; n,m \in \mathbb{N} \}$,  now suppose there is limit point $x \in (\frac{1}{n},\frac{1}{n-1})$. For a given $n \in \mathbb{N}$ either there are a finite number of $m$ corresponding to it or not. If we suppose that there are an infinite number of $(n',m)$ for fixed $n'$ then we have the subset of $A$ that has $\frac{1}{n'}$ as a limit point. This is obvious since for any $\epsilon$ there is certainly $M'$ such that $\frac{1}{m} < \epsilon$ for all $m > M'$ and $(n',m) \in M$. Thus $|\frac{1}{n'}+\frac{1}{m} - \frac{1}{n'}| < \epsilon$ for all $m > M'$ and $(n',m) \in M$. This is a contradiction since we have assumed the limit point to be in $(\frac{1}{n},\frac{1}{n-1})$.\\
\indent The other possibility was that there were only a finite number of $m$ corresponding to a given $n$. Since it is theorem in rudin that there are in infinite number of points in any neighborhood around the limit point there must be an infinite number of $n$ with different $m$ otherwise both $n$ and $m$ are bounded and there are a finite number of points in the neighborhood. It is also true that even though there are a finite number of $m$ corresponding to a given $n$ there must be an infinite number of $m$ in the set otherwise we have an infinite number of $n$ corresponding to a given $m$ which is the case previously handled.\\
\indent The following calculation shows that $n$ and $m$ which both must be unbounded can not be for then for all $n,m > 2N$ we have $\frac{1}{n} + \frac{1}{m} < \frac{1}{N}$ so the remaining points will be outside the interval containing the limit point and thus there exists a neighborhood containing only a finite number of points. We have shown assuming there is a limit point in $(\frac{1}{n},\frac{1}{n-1})$ always leads to a contradiction thus the countable number of limit points shown in the beginning of this problem are the only limit points in $A$. $A$ is thus a bounded set with a countable number of limit points.
