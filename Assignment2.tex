...
We are being asked to show that there is a 1-1 correspondence $g$ such that $g: Z \to \mathbb{R}$. Since $X \backsim \mathbb{R}$ and $Y \backsim \mathbb{N}$ and $X \cap Y = \emptyset$ we can define a function $f$ such that $f : X \to \mathbb{R}$ and $f: Y \to \mathbb{N}$. With this f we have $f(Z) = f(X \cup Y) = f(X) \cup f(Y) = \mathbb{R} \cup \mathbb{N}$ therefore
\[
Z = f^{-1}(\mathbb{R} \cup \mathbb{N})
\]
This shows that finding a 
...
\indent Since $\frac{1}{n} + \frac{1}{m} > 0 $ for all $n$ and $m$ there is no limit point less than 0. Also, 
